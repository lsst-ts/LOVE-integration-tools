...
class RotateATDome(salobj.BaseScript):
    ...
    * "set azimuth:{value}" to rotate the Dome.
    ...
    async def configure(self, config):
        ...
        self.log.info(f"Configure finished, azimuth={self.azimuth}")
    ...
    async def run(self):
        ...
        if not self.remote.start_task.done():
            ...
            self.log.info(f"Starting cmd_moveAzimuth with azimuth={self.azimuth} and timeout={self.cmd_timeout}")
            try:
                await self.remote.cmd_moveAzimuth.set_start(azimuth=self.azimuth, timeout=self.cmd_timeout)
            except TimeoutError:
                self.log.error('cmd_moveAzimuth timeout exceeded')
